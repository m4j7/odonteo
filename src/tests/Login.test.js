...
describe("Tests the login page", () => {
    ...
    it("Should render email error message on screen", ()=> {
        ...
        expect(emailInput).toHaveValue("maju")
    ...
    it("Should render password error message on screen", ()=> {
        ...
        expect(emailInput).toHaveValue("mj@gmail.com")
        ...
        fireEvent.click(loginButton)
        const errorMessage = screen.getByText("Email ou senha com formato incorreto.")
        expect(errorMessage).toBeInTheDocument()
    })
})
...
describe("Verify if can login", () => {
    ...
    it("Should login", async () => {
        ...
        global.fetch(jest.fn(
            async () => ({
                json: async () => Promise.resolve({ user: null, message: {show: false, text: 'Login bem sucedido!', status: '' }, token: null })
            })
        ));
        renderPage(<Login />)
